...
struct Program final : ProgramBase
{
	...
	{
		return Settings("04: Transformations", m_width, m_height);
	}
	...
	virtual void update() final
	{
		...
		projection = glm::perspective(glm::radians(45.0f), static_cast<f32>(m_width) / static_cast<f32>(m_height), 0.1f, 100.0f);
		view = glm::translate(view, glm::vec3(0.0f, 0.0f, -3.0f));
		// pass transformation matrices to the shader
		shaderProgram.setUniformMatrix4f("u_Projection", projection); // note: currently we set the projection matrix each frame, but since the projection matrix rarely changes it's often best practice to set it outside the main loop only once.
